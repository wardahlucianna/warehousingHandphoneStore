...
class m_position_controller extends CI_Controller {
	...
	public function template()
	{
		...
		if($aksi == 'insert_handler' || $aksi == 'edit_handler'){
			...
			$this->db->where('m_position_id',$id);
			$get= $this->db->get();
			$count= $get->num_rows();
	...
	public function data_table()
	{
		...
		$result['row_total']	= $this->db->count_all_results('m_position');
		...
		if($search!=null || $search!=""){
			$this->db->like('a.m_position_name', $search);
			$this->db->or_like('a.m_position_status', $search);
			$this->db->or_like('b.m_position_name', $search);
	...
	public function save()
	{
		...
		$count_name = 0;
		if($code==""){
			$this->db->where("m_position_name",$m_position_name);
			$this->db->from('m_position');
			...
			$this->db->where("m_position_name",$m_position_name);
			$this->db->from('m_position');
			$this->db->where("m_position_id!=",$code);
			$count_name += $this->db->get()->num_rows();
	...
	public function delete(){
		...
		if($count>0){
			...
			$msg 	= "Delete success";
			$this->db->where('m_position_id', $id);
			$data = $this->db->delete('m_position');
		}
