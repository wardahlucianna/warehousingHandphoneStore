...
class m_product_type_controller extends CI_Controller {
	...
	public function template()
	{
		...
		if($aksi == 'insert_handler' || $aksi == 'edit_handler'){
			...
			$this->db->where('m_product_type_id',$id);
			$get= $this->db->get();
			$count= $get->num_rows();
	...
	public function data_table()
	{
		...
		$result['row_total']	= $this->db->count_all_results('m_product_type');
		...
		if($search!=null || $search!=""){
			$this->db->like('m_product_type_name', $search);
			$this->db->or_like('m_product_type_status', $search);
	...
	{
		$code	= $this->input->post('m_product_type_id');
		$m_product_type_name	= $this->input->post('m_product_type_name');
		$m_product_type_status	= $this->input->post('m_product_type_status');
		$m_product_type_status	= $m_product_type_status==true?"Active":"Not Active";

		$this->db->trans_start(); // Query will be rolled back
		$this->db->trans_begin();
		
		$data = array(
			"m_product_type_id"=>$code,
			"m_product_type_name"=>$m_product_type_name,
			"m_product_type_status"=>$m_product_type_status,
		);
		...
		$count_icon = 0;
		if($code==""){
			$this->db->where("m_product_type_name",$m_product_type_name);
			$this->db->from('m_product_type');
			...
			$this->db->where("m_product_type_name",$m_product_type_name);
			$this->db->where("m_product_type_id!=",$code);
			$this->db->from('m_product_type');
			$count_name += $this->db->get()->num_rows();
		...
		if($count_name>0){
			...
			$status = "succsess";
			if($code==""){
				$this->db->insert('m_product_type', $data);
				$msg = "Save success";
			}
			else{
				$this->db->set($data);
				$this->db->where("m_product_type_id",$code);
				$this->db->update('m_product_type');
				$msg = "Update success";
			}
		}
	...
	public function delete(){
		...
		if($count>0){
			...
			$msg 	= "Delete success";
			$this->db->where('m_product_type_id', $id);
			$data = $this->db->delete('m_product_type');
		}
